...
1.	npm install (para descargar e instalar todas las dependencias necesarias del proyecto).
2.	npm run server (en la primera terminal, para iniciar el servidor local que procesa la lógica de datos y simula la base de datos donde se guardan los gastos. Esta consola debe quedar abierta para mantener el servicio activo).
3.	npm run dev (en una segunda terminal en paralelo, para iniciar el servidor de desarrollo de React que renderiza la interfaz visual).
